...
\xfabstract{
    ...
    运动学层面，推导了差速底盘运动学方程，对直线、圆弧、S形和8字形四类典型轨迹开展了仿真。路径规划层面，采用Hybrid A*算法在仓储栅格地图上规划了无碰撞路径，路径长度为10.90m。循迹控制层面，模拟5路红外传感器阵列进行PID循迹仿真，RMS横向误差为3.1cm。最后设计了双轮差速PID控制器，在扰动条件下进行目标跟踪仿真，检验了控制器的扰动抑制能力。
