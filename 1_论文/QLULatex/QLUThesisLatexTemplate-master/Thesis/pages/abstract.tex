...
\xfabstract{
    ...
    电子商务与智能物流的快速推进，使传统仓储搬运方式在效率和成本上遭遇日益严峻的考验。着眼于仓储搬运自动化水平的提升，本文对智能仓储场景下的举升式AGV小车展开研究。

    经过三种驱动方案的对比，选定了以双轮差速驱动为核心的举升式AGV总体方案。机械结构层面，完成了底盘、驱动轮、升降机构和罩壳的设计，并借助有限元分析对支撑梁进行了验证。电气系统层面，选定STM32F103C8T6作为主控芯片，完成了伺服电机与减速器的选型匹配，设计了红外和超声波传感器的接口电路。

    运动学层面，推导了差速底盘运动学方程，对直线、圆弧、S形和8字形四类典型轨迹开展了仿真。路径规划层面，采用Hybrid A*算法在仓储栅格地图上规划了无碰撞路径，路径长度为11.03m。循迹控制层面，模拟5路红外传感器阵列进行PID循迹仿真，RMS横向误差为2.1cm。最后设计了双轮差速PID控制器，在扰动条件下进行目标跟踪仿真，检验了控制器的扰动抑制能力。

    测试结果显示，所提出的AGV方案在结构设计、路径规划与运动控制上均切实可行，可为后续样机研制提供参考。
