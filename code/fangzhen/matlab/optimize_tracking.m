%% 循迹PID参数优化 —— 扫描Kp/Kd/车速/滤波窗口组合
%  以Hybrid A*规划路径为参考, 逐组运行循迹仿真, 输出最优参数
function optimize_tracking()
addpath(fileparts(mfilename('fullpath')));
ref_path = c_hybrid_astar();  % 获取规划路径

%% 参数搜索范围
Kp_list      = [3, 4, 5, 6];
Kd_list      = [0.8, 1.0, 1.5, 2.0];
v_list       = [0.20, 0.25, 0.30];
ma_list      = [6, 8, 12];
    ...
    for kd = Kd_list
        for v = v_list
            for ma = ma_list
                ...
                if mod(idx, 20) == 0
                    fprintf('  进度 %d/%d, 当前最优 RMS=%.3f cm\n', idx, total, best_rms*100);
                end
            end
        end
    end
...
fprintf('\n========== 优化结果 ==========\n');
fprintf('最优 Kp=%.1f, Ki=0.08, Kd=%.1f, v=%.2f m/s, ma_window=%d\n', best_cfg);
fprintf('最优 RMS: %.3f cm\n', best_rms*100);
