...
% best_g(r, c, tb) 记录该栅格+航向bin下的最优g值
...
for iter = 1:max_iter
    ...
    if dist_to_goal < step_len * 1.5  % 距目标较近时尝试直连
        dtheta = goal_pos(3) - ct;
        dtheta = atan2(sin(dtheta), cos(dtheta));  % 归一化
        if abs(dtheta) < goal_ang_tol  % 航向也接近
            if ~check_obs(cx, cy, goal_pos(1), goal_pos(2), obs_map, res, rows, cols)
                ...
                elapsed = toc(t_start);
                path = weighted_avg_smooth(path, 5);  % 加权均值平滑
                print_metrics(path, iter, n_expanded, elapsed, obstacles, res, rows, cols, r_robot);
    ...
    for a = 1:n_curv*2  % 10种动作(5曲率×前进/后退)
        ...
        g = g + 0.05 * heading_diff;  % 舌向偏差小则代价低
        ...
        if g >= best_g(nr, nc, ntb)  % 已有更优解
            continue;
        end
        best_g(nr, nc, ntb) = g;

        % 检查CloseList（仅当栅格级去重不足以排除时）
...
function print_metrics(path, n_iter, n_expanded, elapsed, obstacles, ~, ~, ~, r_robot)
    ...
    fprintf('扩展节点: %d\n', n_expanded);
    fprintf('搜索耗时: %.3f s\n', elapsed);
    fprintf('节点效率: %.4f m/节点 (路径长度/扩展节点)\n', total_len/max(n_expanded,1));
