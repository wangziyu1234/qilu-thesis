...
for i = 1:Ncase
	[v_cmd, omega_cmd] = cases{i}.u(t(1:end-1));
	sim = simulate_diff_drive(v_cmd, omega_cmd, t, x0, r, L);
	sim.name = cases{i}.name;
	sim.color = cases{i}.color;
	sim.metrics = analyze_metrics(sim, dt);
	results{i} = sim;
end
...
for i = 1:Ncase
	...
	plot(s.x(1), s.y(1), 'o', 'Color', s.color, 'MarkerFaceColor', s.color, 'HandleVisibility', 'off');
	plot(s.x(end), s.y(end), 's', 'Color', s.color, 'MarkerFaceColor', s.color, 'HandleVisibility', 'off');
end
...
% 小车外形参数（简化为圆盘 + 航向箭头）
car_r = 0.10;
car_outline_x = car_r * cos(linspace(0, 2*pi, 50));
car_outline_y = car_r * sin(linspace(0, 2*pi, 50));
heading_len = 0.18;
margin = 0.3;

% 每个子图中的图元句柄
car_body = gobjects(Ncase, 1);
car_center = gobjects(Ncase, 1);
car_heading = gobjects(Ncase, 1);
car_trail = gobjects(Ncase, 1);
...
for i = 1:Ncase
	...
	% 背景参考轨迹
	plot(s.x, s.y, '--', 'Color', [0.75, 0.75, 0.75], 'LineWidth', 1.0, 'DisplayName', '参考轨迹');
...
for k = 1:step:numel(t)
	...
	if gif_enabled
		if ~isgraphics(fig_anim, 'figure')
			warning('动画窗口句柄无效，已停止 GIF 导出，仅保留动画播放。');
			gif_enabled = false;
			continue;
		end
...
for i = 1:Ncase
	...
	m = results{i}.metrics;
	path_len(i) = m.path_len;
	disp_len(i) = m.disp_len;
	avg_v(i) = m.avg_v;
	max_abs_omega(i) = m.max_abs_omega;
	max_abs_kappa(i) = m.max_abs_kappa;
	final_heading_deg(i) = m.final_heading_deg;
end
...
path_len = sum(hypot(dx, dy));
disp_len = hypot(sim.x(end) - sim.x(1), sim.y(end) - sim.y(1));
avg_v = mean(abs(sim.v_cmd));
max_abs_omega = max(abs(sim.omega_cmd));
max_abs_kappa = max(abs(sim.kappa));
final_heading_deg = rad2deg(sim.theta(end));
